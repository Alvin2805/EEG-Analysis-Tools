/*#############################################################################
                       ...
                       FFT_COH2_EEG(int argc, char *argv[ ])
...
int MAT, i, j, total_data, total_electrodes, start, end, time, EEG_CH1, EEG_CH2;
...
char fin[100], fout_p[100], fout_ri[100], fout[100], fin_1[100], fin_2[100], fin_px[100], fin_py[100], fin_xsp[100];

FILE *FIN, *FOUT_P, *FOUT_RI, *FOUT, *FIN_CH1, *FIN_CH2, *FIN_PX, *FIN_PY, *FIN_XSP;
...
/*################FFT Calculation for EEG_CH1################*/
    ...
    for(i=0;i<(total_data / INTERVAL);i++){
      ...
      i_data = 0;
      for(j=start;j<end;j++){
	xr[i_data] = data[j][EEG_CH1] - ave;
	i_data++;
      }
      n=i_data;

      i_data=1;
      while(( (int)pow(2,i_data) % n ) != 0) i_data++;
      m=i_data;
	...
	p = pp;
      ...
      sprintf(fout_p,"%s_ch%d_%d-%d.fftp",argv[1],EEG_CH1,start,end);
      FOUT_P = fopen(fout_p, "w");
      sprintf(fout_ri,"%s_ch%d_%d-%d.fftri",argv[1],EEG_CH1,start,end);
...
/*################FFT Calculation for EEG_CH2################*/
	...
	p = pp;
      ...
      sprintf(fout_p,"%s_ch%d_%d-%d.fftp",argv[1],EEG_CH2,start,end);
      FOUT_P = fopen(fout_p, "w");
      sprintf(fout_ri,"%s_ch%d_%d-%d.fftri",argv[1],EEG_CH2,start,end);
      FOUT_RI = fopen(fout_ri, "w");
    ...
    for(i=0;i<(total_data / INTERVAL);i++){
      ...
      sprintf(fin_1,"%s_ch%d_%d-%d.fftri",argv[1],EEG_CH1,start,end);
      sprintf(fin_2,"%s_ch%d_%d-%d.fftri",argv[1],EEG_CH2,start,end);
    ...
    for(i_p=0;i_p<(INTERVAL/2);i_p++) yy[i_p] = 0;
    i_f = 0;
    ...
    for(i=0;i<(total_data / INTERVAL);i++){
      ...
      sprintf(fin,"%s_ch%d_%d-%d.fftp",argv[1],EEG_CH1,start,end);
    ...
    sprintf(fout,"%s-%d_ch%d.linuxcsd.px",argv[1],time,EEG_CH1);
    ...
    for(i=0;i<(total_data / INTERVAL);i++){
      ...
      sprintf(fin,"%s_ch%d_%d-%d.fftp",argv[1],EEG_CH2,start,end);
    ...
    sprintf(fout,"%s-%d_ch%d.linuxcsd.py",argv[1],time,EEG_CH2);
    ...
    for(i_p=0;i_p<1000;i_p++) yy[i_p] = 0;
    i_f = 0;
    ...
    sprintf(fout,"%s-%d_ch%dch%d.linuxcsd.xsp",argv[1],time,EEG_CH1,EEG_CH2);
    ...
    /*################Coherence & Phase################*/
     ...
     sprintf(fin_px,"%s-%d_ch%d.linuxcsd.px",argv[1],time,EEG_CH1);
     sprintf(fin_py,"%s-%d_ch%d.linuxcsd.py",argv[1],time,EEG_CH2);
     sprintf(fin_xsp,"%s-%d_ch%dch%d.linuxcsd.xsp",argv[1],time,EEG_CH1,EEG_CH2);
     ...
     sprintf(fout,"%s-%d_ch%dch%d.linuxcsd",argv[1],time,EEG_CH1,EEG_CH2);
     FOUT = fopen(fout,"w");
...
void Manual(int argc){
    if(argc != 5){
        ...
        printf("                        ./FFT_COH2 <filename(-#.linuxcsd)> <EEG_CH1> < EEG_CH2> <time>\n\n");
        exit(1);
    }
}
